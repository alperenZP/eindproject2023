...
if (isset($_GET['reduce'])) {
    if ($_SESSION["questions_add_amount"] > 0) {
        $_SESSION["questions_add_amount"] -= 1;
    }
    echo $_SESSION["questions_add_amount"];
    exit();
}
...
<head>
    ...
    <script>
        function reduceQuestionsAddAmount() {
            const xhr = new XMLHttpRequest();
            xhr.open('GET', '?reduce=1', true);
            xhr.onreadystatechange = function () {
                if (xhr.readyState == 4 && xhr.status == 200) {
                    console.log(xhr.responseText); // Optional: Update the UI based on response
                }
            };
            xhr.send();
        }
            ...
            <div class="flex flex-col gap-4">
                ...
                for ($y = 0; $y < count($questions); $y++) {
                    ...
                    <div class="card w-96 bg-base-100 shadow-xl">
                        <div class="card-body">
                            <a href="https://bibliotheek.live/alperenGit/src/lib/user/teacher/delete_question.php?questionid=' . $questions[$y]["id"] . '"><button class="btn btn-error">❌</button></a>
                            <br><br>
                ...
                for ($x = 0; $x < $_SESSION["questions_add_amount"]; $x++) {
                    ...
                    <div class="card w-96 bg-base-100 shadow-xl">
                        <div class="card-body">
                            <button type="button" class="btn btn-error" onclick="reduceQuestionsAddAmount()">ERGZ</button>
