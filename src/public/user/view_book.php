...
$chapters = fetch_as_array(
    'SELECT *, book_chapters.title AS "chapterTitle", books.id AS "bookid", book_chapters.id AS "chapterid" FROM `book_chapters` INNER JOIN books ON (books.id = book_chapters.bookid) WHERE bookid = ? ORDER BY book_chapters.id ASC',
    ['type' => 'i', 'value' => $_GET["book"]]
);
...
<div class="min-h-[100svh] w-full flex flex-col justify-center items-center p-8">
    ...
    <ul class="menu menu-horizontal bg-base-200 w-400">
        ...
        <li><a href="https://bibliotheek.live/alperenGit/src/public/user/view_exercises.php?bookid=<?php echo $_GET["book"]?>">Toetsen</a></li>
                ...
                foreach ($chapters as $chapter) {
                    ...
                    if ($_SESSION['user']['isTeacher']) {
                        echo '
                            ...
                            <td><a href="https://bibliotheek.live/alperenGit/src/lib/user/teacher/delete_chapter.php?id=' . $chapter["chapterid"] . '"><button class="btn btn-error">❌</button></a></td>
                        ';
                    }
                    echo '
                        </tr>
    ...
    if (count($chapters) == 0) {
        echo '
                <div role="alert" class="alert alert-info">
                    ...
                    <span>Er staan geen hoofdstukken hier.</span>
                </div>
            ';
    }
    ?>
